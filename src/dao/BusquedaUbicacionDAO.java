...
public class BusquedaUbicacionDAO {
    ...
    private String sqlObtenerDatosPorCoordenada = "SELECT e.titulo, e.anio_publicacion, e.descripcion, e.cantidad_total, e.fecha_ingreso  FROM ejemplar AS e INNER JOIN ubicacion AS u ON e.id_ubicacion = u.id_ubicacion  WHERE u.edificio = ?  AND u.piso = ?  AND u.seccion = ?  AND u.estante = ?";
    ...
    public ArrayList<String> obtenerDatoCoordenada(String dato) {
        ...
        try (
            ...
            log.error("error al obtener ubicaciones", e);
        }
        return listaDatos;
    }
    ...
    public ArrayList<Object> obtenerDatosPorCoordenadas(String edificio, String piso, String seccion, String estante) {
        ...
        try (
            ...
            obtenerMaterial.setString(1, edificio);
            obtenerMaterial.setString(2, piso);
            obtenerMaterial.setString(3, seccion);
            obtenerMaterial.setString(4, estante);

            try (ResultSet res = obtenerMaterial.executeQuery();) {
                while (res.next()) {
                    ArrayList<String> camposMaterial = new ArrayList<>();
                    camposMaterial.add(res.getString("titulo"));
                    camposMaterial.add(res.getString("anio_publicacion"));
            ...
            log.error("Error al obtener datos por coordenada", e);
        }
